...
const modelName = '/new_balance_997';
const modelSuffixes = ['_outer', '_inner', '_sole', '_laces'];
...
function setModel() {
	...
	var materials = [];
	var modelSuffixes = ['_outer', '_inner', '_sole', '_laces'];
	var loader = new THREE.TextureLoader();
	for ( let i = 0; i < 4; i++ )
	{
		var material = new THREE.MeshStandardMaterial();
		material.map = loader.load( path + res + modelName + modelSuffixes[i] + '_diff_' + res + '.jpg' );
		material.normalMap = loader.load( path + res + modelName + modelSuffixes[i] + '_nor_gl_' + res + '.jpg' );
		material.roughnessMap = loader.load( path + res + modelName + modelSuffixes[i] + '_rough_' + res + '.jpg' );
		material.roughness = 1.0;
		material.metalnessMap = loader.load( path + res + modelName + modelSuffixes[i] + '_metal_' + res + '.jpg' );
		material.metalness = 1.0;
